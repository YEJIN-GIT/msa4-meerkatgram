...
@Entity // 해당 클래스가 JPA 엔티이임을 선언
@EntityListeners(AuditingEntityListener.class)  // 엔티이에 이벤트 리스너 지정
...
public class User {
    ...
    @JdbcTypeCode(Types.VARCHAR)    // Enum객체 사용시 JdbcTypeCode로 별도 타입설정 필요
    private ProviderPolicy provider = ProviderPolicy.NONE;  // 필드 초기화 할때 디폴트 값 넣어둠.
