...
public class SecurityConfiguration {
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        // 브라우저가 preflight  요철 결과를 캐시할 시간(초 단위) 설정
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, SecurityExceptionHandler securityExceptionHandler, TokenAuthenticationFilter tokenAuthenticationFilter) throws Exception {
        return http
            ...
            .formLogin(AbstractHttpConfigurer::disable) // 폼로그인(스프링 시큐리티가 자동으로 만들어 주는 로그인페이지) 기능 비활성 설정
            ...
            .authorizeHttpRequests(req ->
                // 리퀘스트에 대한 권한 설정
                req.requestMatchers(HttpMethod.GET, SecurityUrlRegistry.AUTH_REQUIRED_GET_URLS).authenticated() // 인증 필요
